...
import { makeShowSuccessCode, makeShowSuccessMessage, makeShowErrorCode, makeShowErrorMessage, makeShowOTLists, makeSelectUserData } from './selector';

import reducer from './reducer';
import saga from './saga';

import { createFOrms, dismissForms, retrieveForms, resetState } from './action';

export class DTRRequests extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  constructor(props) {
    super(props);
    this.state = {
  ...
  onSubmitOTRequest = (totalActualHours, empid) => {
    ...
    if (!error) {
      const data = {
        RequestorEmpID: empid,
        RequestedMinutes: requestedOTMinutes,
        ShiftDate: moment(start).format('YYYY-MM-DD'),
        // RenderedMinutes: totalActualHours,
        Remarks: remarks,
      };
      this.props.onCreateForms(data);
    }
  }
  // Get remarks value
  onGetRemarks = (e) => {
  ...
  handleChangeStart = (date) => {
    ...
    this.props.dataRetrieveLists(moment(date).format('YYYY-MM-DD'));
    console.log(this.props.dataRetrieveLists)
  }
        ...
        >
          <CreateNewForm>
            <Fields>
              ...
              <div className="half">
                <label htmlFor="rendered-hours">Active Minutes</label>
                {OTLists}
              </div>
            </Fields>
            <Fields>
              <label htmlFor="ot-hours">Requested Minutes</label>
            ...
            <ButtonWrapper>
              ...
              <Button handleRoute={(e) => { e.preventDefault(); this.onSubmitOTRequest(totalActualHours, showUserData.EmpProfileID); }} color="gray">SUBMIT</Button>
...
DTRRequests.propTypes = {
  close: PropTypes.func,
  onCreateForms: PropTypes.func,
  dismissSuccessMessage: PropTypes.func,
  dataRetrieveLists: PropTypes.func,
  ...
  showUserData: PropTypes.oneOfType([
    PropTypes.number,
    PropTypes.bool,
  ]),
...
const mapStateToProps = createStructuredSelector({
  ...
  showUserData: makeSelectUserData(),
});

function mapDispatchToProps(dispatch) {
  return {
    onCreateForms: (data) => dispatch(createFOrms(data)),
    dataRetrieveLists: (startdate) => dispatch(retrieveForms(startdate)),
    dismissSuccessMessage: () => dispatch(dismissForms()),
...
export default compose(withReducer, withSaga, withConnect)(DTRRequests);

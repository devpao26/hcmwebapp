...
class EmpListComponent extends React.PureComponent {
  select = (e, empProfileID, empFirstName, empLastName, requestor) => {
    ...
    this.props.selectEmp(empProfileID, empFirstName, empLastName, requestor);
  }
  render() {
    ...
    if (lists) {
      items = lists.map((item) => (
        <dl role="presentation" key={item.EmpProfileID} className="cont" onClick={(e) => { this.select(e, item.EmpProfileID, item.FirstName, item.LastName, 1); }}>
          <dt>

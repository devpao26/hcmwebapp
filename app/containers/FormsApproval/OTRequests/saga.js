 ...
 */
export function* getOTRequestList() {
  ...
  try {
    ...
    if (resp.ResponseCode === 200) {
      ...
      yield delay(500);
      yield put(getOTRequestsSuccess(data, pages));
    }
    ...
    if (resp.ErrorCode !== 0) {
      if (resp.ErrorCode === 401) {
        ...
        yield put(getOTRequestsSuccess(false, false));
        yield put(getOTRequestsError(resp));
      } else {
        yield put(getOTRequestsError(resp));
      }
    }
  } catch (err) {
    yield put(getOTRequestsError(err));
 ...
 */
export default function* otRequestsSagas() {
  ...
  // actions
  yield takeLatest(GET_OTLISTS, getOTRequestList);

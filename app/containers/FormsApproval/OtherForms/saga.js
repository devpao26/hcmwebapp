...
/**
 * Custom Forms Request List request/response handler
 */
export function* getRequestList() {
  ...
  try {
    ...
    if (resp.ResponseCode === 200) {
      ...
      yield delay(500);
      yield put(getListRequestsSuccess(data, pages));
    }
    ...
    if (resp.ErrorCode !== 0) {
      if (resp.ErrorCode === 401) {
        ...
        yield put(getListRequestsSuccess(false, false));
        yield put(getListRequestsError(resp));
      } else {
        yield put(getListRequestsError(resp));
      }
    }
  } catch (err) {
    yield put(getListRequestsError(err));
...
export default function* coeRequestsSagas() {
  ...
  // actions
  yield takeLatest(GET_LIST, getRequestList);

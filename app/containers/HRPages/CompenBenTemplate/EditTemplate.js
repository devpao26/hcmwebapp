...
class EditTemplateForm extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      isFormError: false,
  ...
  componentWillMount() {
    ...
    if (this.props.details) {
      savedAllowance = this.props.details.CompbenAllowancesList;
      savedBenefits = this.props.details.CompBenefitsList;

      savedAllowance.map((item) => {
        allowance.push({
          id: item.CompBenAllowanceID,
          Name: item.Name,
      ...
      savedBenefits.map((item) => {
        benefits.push({
          id: item.CompBenefitID,
          CompBenefitTypeID: item.CompBenefitTypeID,
          Name: item.CompBenefitType.Name,
          Value: item.Value,
          isTypeError: false,
          isValueError: false,
        });
      ...
      this.setState({
        templateName: this.props.details.Name,
        basicSalary: this.props.details.BasicSalary,
        isDailyRate: this.props.details.IsDailyRate,
        allowanceFields: allowance,
        benefitFields: benefits,
      });
    }
  }
  ...
  onChangeBenefitType = (e, index) => {
    ...
    const list = this.props.benefitTypes;
    const itemIndex = list.findIndex((obj) => obj.Name === e.currentTarget.value);

    const id = this.props.benefitTypes[itemIndex].CompBenefitTypeID;
    const name = this.props.benefitTypes[itemIndex].Name;

    this.state.benefitFields[index].CompBenefitTypeID = id;
    this.state.benefitFields[index].Name = name;
  ...
  onSubmit = (e) => {
    ...
    this.state.benefitFields.map((item, index) => {
      ...
      const list = this.props.details.CompBenefitsList;
      const savedIndex = list.findIndex((obj) => obj.CompBenefitID === item.id);

      if (savedIndex !== -1) {
        CompBenefitsList.push({
          CompBenefitID: item.id,
          CompBenefitTypeID: item.CompBenefitTypeID,
          Value: item.Value,
        });
      } else {
    ...
    this.state.allowanceFields.map((item, index) => {
      ...
      const list = this.props.details.CompbenAllowancesList;
      const savedIndex = list.findIndex((obj) => obj.CompBenAllowanceID === item.id);
      if (savedIndex !== -1) {
        CompbenAllowancesList.push({
          CompBenAllowanceID: item.id,
          Name: item.Name,
          UpdateIsTaxable: item.IsTaxable,
          Value: item.Value,
        });
      } else {
        CompbenAllowancesList.push({
    ...
    if (!formErrors) {
      const data = {
        ...
        UpdateIsDailyRate: this.state.isDailyRate,
        CompBenefitsList,
        CompbenAllowancesList,
      };

      this.props.save(data, false);
    }
  }
  ...
  addBenefit = (e) => {
    ...
    this.setState((prevState) => ({
      benefitFields: [...prevState.benefitFields, { id, CompBenefitTypeID: '', Value: 0, Name: '', isTypeError: false, isValueError: false }],
    }));
  }
  ...
  render() {
    ...
    if (benefitTypes) {
      benefitTypesItems = benefitTypes.map((item) =>
        <option key={item.CompBenefitTypeID} value={item.Name}>{item.Name}</option>
      );
    }
      ...
      <Fields key={item.id} noPaddingTop>
        ...
        <div className="one-third">
          <span className="select-custom">
            ...
            <select defaultValue={(item.IsTaxable) ? 'Taxable' : 'Non Taxable'} onChange={(e) => this.onChangeAllowanceType(e, index)}>
    ...
    const benefitItems = this.state.benefitFields.map((item, index) => (
      <Fields key={item.id} noPaddingTop>
        ...
        <div className="two-third">
          <span className="select-custom">
            <i className="fa fa-caret-down" />
            <select defaultValue={(item.Name !== '') ? item.Name : 'none'} onChange={(e) => this.onChangeBenefitType(e, index)} className={item.isTypeError && 'error'}>
    ...
    return (
      <CreateNewForm>
        ...
        <Fields>
          ...
          <input value={this.state.templateName} type="text" id="templateName" placeholder="New Work Status Template Name" onChange={(e) => this.onChangeTemplateName(e)} className={this.state.templateNameError && 'error'} />
        </Fields>
        <Fields>
          {/* <div className="one-third">
          ...
          <div className="one-third last">
            ...
            <input value={this.state.basicSalary} id="basicPay" type="number" placeholder="Php 15,000.00" onChange={(e) => this.onChangeBasicPay(e)} className={this.state.basicSalaryError && 'error'} />

  ...
  render() {
    ...
    if (lists) {
      ...
      items = lists.map((item, index) =>
        <p role="presentation" key={item.CompBenTemplateID} className={(index === templateIndex) ? 'active list-item' : 'list-item'} onClick={(e) => { this.selectTemplate(e, item.CompBenTemplateID); }}>{item.Name}</p>
      );

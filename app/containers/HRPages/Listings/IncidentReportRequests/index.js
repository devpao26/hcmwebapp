...
import IncidentReportRequests from './Lists';

export class IncidentRequestComponent extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  constructor(props) {
    super(props);
    this.state = {
      searchVal: '',
  ...
  render() {
    ...
    if (refLists && this.state.isViewDetails) {
      if (requestDetails) {
        if (requestDetails.IRFRequestAttachList.length !== 0) {
          attachs = requestDetails.IRFRequestAttachList.map((attach) =>
    ...
    return (
      <ListContainer>
        <h3>Incident Report Requests List</h3>
        ...
        <Listings noPadding>
          <IncidentReportRequests {...refLists} />
        </Listings>
        ...
        <Modal
          ...
          showCloseBtn
          title="Incident Report Request Details"
          width="400px"
        >
          {(requestDetails) &&
            <FormViewDetails>
              { (requestDetails.FormRequest.FormRequestStatusID === FORMREQUEST_REJECT && requestDetails.FormRequest.ApproverPassedStep !== null) &&
                <p>
              ...
              <p>
                ...
                <span className="value">{requestDetails.IRFType.Name}</span>
              </p>
              <p>
              ...
              <p>
                ...
                <span className="value">{moment(requestDetails.RequestDate).format('LL')}</span>
              </p>
              <p>
                <span className="label">Time Requested</span>
                <span className="value">{requestDetails.RequestTime}</span>
              </p>
              <p>
                <span className="label">Location</span>
                <span className="value">{requestDetails.RequestLocation}</span>
              </p>
              <p>
...
IncidentRequestComponent.propTypes = {
...
const withReducer = injectReducer({ key: 'IRFRequests', reducer });
const withSaga = injectSaga({ key: 'IRFRequests', saga });

            ...
            <div className="enrolled-items" key={item.EmpProfileID}>
              <p>
                ...
                <span className="enrolled-name">
                  ...
                  {(item.WorkStatusTemplate.TemplateInheritance.InheritedFromID !== DEFAULT_GUID) && <small className="enrolled-inherit">Template inherited from {item.WorkStatusTemplate.TemplateInheritance.RefType} - {item.WorkStatusTemplate.TemplateInheritance.Name}</small>}
                </span>
              </p>
              {(item.WorkStatusTemplate.TemplateInheritance.InheritedFromID === DEFAULT_GUID) && <button className="btn-delete" onClick={(e) => { this.props.unassign(e, `${item.LastName} ${item.FirstName}`, item.EmpProfileID); }}><FontAwesomeIcon icon={faMinus} /></button>}
            </div>
          );
        });
      }
